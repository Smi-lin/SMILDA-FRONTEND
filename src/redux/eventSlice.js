...
export const getAllEventsShop = createAsyncThunk(
    ...
    async (id, { rejectWithValue }) => { // Add the 'id' parameter here
      try {
        const { data } = await axios.get(`${event}/getAllEventShop/${id}`); // Use 'id' in the URL
        return data.events;
      } catch (error) {
        return rejectWithValue(error.response.data.message);
      }
...
export const deleteEvent = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await axios.delete(`${event}/deleteEvents/${id}`, {
